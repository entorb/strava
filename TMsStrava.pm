...
# in: $token, $id of activity, $txt [0,1] -> 1 generates .txt files from the jsons
    ...
    my $cont
        = getContfromURL(
        "$o{'urlStravaAPI'}/activities/$id?include_all_efforts=true",
        $token );
...
# check for each activity which parameters are present, to ensure that all parameters are included
 ...
 # You should do this for every worksheet you want to autofit.
    $worksheet->add_write_handler( qr[\w],
        \&excel_store_excel_string_widths );
...
# 2018-08-28 or 2018-08-28T14:24:22+00:00 or 2018-08-28T14:24:22Z or 20180828T142422Z
        my $index;
        ($index)
            = grep { $Reihenfolge[ $_ ] eq "start_date_local" }
            0 .. $#Reihenfolge;
        $worksheet->write_date_time( $line, $index, $h{"start_date_local"},
            $formatDate );
        ($index)
            = grep { $Reihenfolge[ $_ ] eq "start_date" } 0 .. $#Reihenfolge;
...
sub zipFiles {
    ...
    zip \@files    => $pathToZip,
        ...
        ,
        ZipComment =>
        "Created by Torben's Stava App https://entorb.net/strava"
...
sub fetchSegmentLeaderboard {
    my ( $token, $segment_id, $date_range, $club_id, $gender, $age_group )
        = @_;
    ...
    while ( $lastpage != 1 and $page <= 10 ) {
        ...
        foreach my $hashref (@entries_this_page) {
            my %h2      = %{$hashref};
            my $listref = [
                $h2{"rank"}, $h2{"elapsed_time"}, $h2{"athlete_name"},
                formatDate( $h2{"start_date_local"}, 'date' )
            ];
            push( @list, $listref );
...
sub htmlPrintNavigation {
    ...
    if ( -f $s{'pathToActivityListHashDump'} ) {
        my @allActivityHashes
            = @{ retrieve( $s{'pathToActivityListHashDump'} ) };    # retrieve data from file (as ref)
...
# first line is ignored (header)
    ...
    while ( my $line = <$fhIn> ) {
        ...
        my ($continent, $country,  $subdivision,
