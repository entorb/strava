...
# DESCRIPTION
# Package for boundling all methods for my stava app
...
our @EXPORT = qw( %o %s);       # o = Global Sessings / Options ; s = Session variables
...
sub logSubStart {
  ...
  # in: $str to pass to log ( in log the check if $s{'write-session-log'} == 1 is perforemed)
  my ( $str ) = @_;
  # logSubStart ('logIt');
...
sub initSessionVariables {
  ...
  if ( $s{ 'write-session-log' } == 1 ) {
    ...
    open my $fhIn, '>>', $fileLog or die "ERROR: can't write to session logfile'";
    print { $fhIn } "\n\n\n\n";
    $s{ 'FhSessionLog' } = $fhIn;
  ...
  ( $s{ 'stravaUserID' }, $s{ 'stravaUsername' }, $s{ 'token' }, $s{ 'scope' } ) = @cont;
  $s{ 'fileKnownLocations' } = "$o{'dirKnownLocationsBase'}/$s{'stravaUserID'}.txt";
  # return @cont; # ($stravaUserID,$stravaUsername,$token,$scope)	# not used any more, now this is strored in %s
  return;
...
sub deauthorize {
  ...
  my ( $htmlcode, $cont ) = PostPutJsonToURL( 'POST', "https://www.strava.com/oauth/deauthorize", $token, $silent );
  # my %h = convertJSONcont2Hash($cont);
  return;
} ## end sub deauthorize
...
sub fetchActivityListYear {
  ...
  logSubStart( 'fetchActivityListYear' );
  my $page         = 1;
  my $timestampnow = time;
  my ( $secsToday, $timestampDaysOld );
  my $param = "per_page=$per_page";
...
sub fetchActivitySingle {
  ...
  my $cont = getContfromURL( "$o{'urlStravaAPI'}/activities/$id?include_all_efforts=true", $token );
  ...
  my $fileOut = "$s{'tmpDataFolder'}/activitySingle/" . $date . "-" . $h{ "type" } . "-$id" . ".json";
  $_ = dirname( $fileOut );
  make_path $_ unless -d $_;
...
sub getContfromURL {
  ...
  $cont = decode( 'UTF-8', $cont );    # for some reason this is required and not included in $res->decoded_content
...
sub convertJSONcont2Array {
  ...
  die "E: message '$decoded' is no ARRAYREF" if ( not ref( $decoded ) eq "ARRAY" );
...
sub ref2String {
  ...
  #  else $field is returned untoucht
  my ( $field ) = @_;
...
sub convertJsonFilesToArrayOfHashes {
  ...
  foreach my $fileIn ( @L ) {
    ...
    if ( $cont =~ m/^\[\{/ ) {    # for activityList the decoded JSON is a List -> ARRAYREF
      my @activitiesOfThisFile = convertJSONcont2Array( $cont );
      push @allActivityHashes, @activitiesOfThisFile;
    } elsif ( $cont =~ m/^\{/ ) {    # for single activity the decoded JSON is a Hash -> HASHREF
      my %h = convertJSONcont2Hash( $cont );
...
sub getKnownLocationsOfUser {
  ...
  @knownLocations = ( [ 49.574986, 10.967483, "ER-Schaeffler" ], [ 51.070298, 13.760067, "DD-Alaunpark" ], [ 53.330333, 10.138152, "P-MTV-Pattensen" ], [ 51.010218, 13.701419, 'DD-Robotron' ], [ 49.60579, 11.036603, 'ER-MeilwaldWiese' ] );
...
sub convertActivityHashToExcel {
  ...
  my $workbook = Excel::Writer::XLSX->new( $fileOutExcel ) or die "ERROR: Can't open $fileOutExcel for writing!\n";
  $workbook->set_properties(
    title    => 'Strava Excel Activity Export',
    author   => 'Torben Menke',
    comments => 'https://entorb.net/strava/ created with Perl and Excel::Writer::XLSX',
    category => 'Sport'
  );
  my $worksheet       = $workbook->add_worksheet( "ActivityListData" );
  my $formatHeaderRow = $workbook->add_format( bold => 1 );               # color => 'red'

  my $formatDate = $workbook->add_format( num_format => 'dd.mm.yyyy hh:mm:ss' )    #
  ...
  push @Reihenfolge, grep { not $in_R{ $_ } } sort keys %hashOfActivitiyParameters;
  ...
  foreach my $activity ( @allActivityHashes ) {
    ...
    # TODO: overwrite data formated using date format
    ...
    $worksheet->write_date_time( $line, $index, $h{ "start_date_local" }, $formatDate );
    ( $index ) = grep { $Reihenfolge[ $_ ] eq "start_date" } 0 .. $#Reihenfolge;
    $worksheet->write_date_time( $line, $index, $h{ "start_date" }, $formatDate );

  } ## end foreach my $activity ( @allActivityHashes)

  # Run the autofit after you have finished writing strings to the workbook.
  excel_autofit_columns( $worksheet );    # from # https://metacpan.org/pod/Spreadsheet::WriteExcel::Examples#Example:-autofit.pl
  $workbook->close;
  return;
...
sub convertFetchedActivityListJsonFilesToExcel {
  ...
  # calle convertActivityHashToExcel
  # in: $fileNameExcel
  ...
  my @allActivityHashes = convertJsonFilesToArrayOfHashes( @L );
  convertActivityHashToExcel( $fileNameExcel, $refKnownLocations, @allActivityHashes );
  return;
...
sub fetchSegmentRecord {
  ...
  my $cont        = getContfromURL( "$o{'urlStravaAPI'}/segments/$segmentid/leaderboard?per_page=1&page=1", $token );
  my %h           = convertJSONcont2Hash( $cont );
...
sub htmlPrintNavigation {
  ...
  my $missingActivityCacheDisablesButton      = -f $s{ 'pathToActivityListHashDump' } ? '' : ' disabled="disabled"';
  my $missingScopeActivityWriteDisablesButton = $s{ 'scope' } =~ m/activity:write/    ? '' : ' disabled="disabled"';

  my $countActCached = 0;
  if ( -f $s{ 'pathToActivityListHashDump' } ) {
    my @allActivityHashes = @{ retrieve( $s{ 'pathToActivityListHashDump' } ) };    # retrieve data from file (as ref)
    $countActCached = 1 + $#allActivityHashes;
    undef @allActivityHashes;
  }
  say "<center><p><small>cached activities: $countActCached</small></p></center>";
...
(requires caching first)" />
	...
	<input type="submit" name="submitFromNav" class="navButton" id="btnNavActTop10" value="Activity Top10" ' . $missingActivityCacheDisablesButton . ' 
...
(requires caching first)" />
	...
	<input type="submit" name="submitFromNav" class="navButton" id="btnNavActSearch" value="Search For Activity" ' . $missingActivityCacheDisablesButton . '
...
';
  say '
	...
	<input type="submit" name="submitFromNav" class="navButton" id="btnNavActListExcelImport" value="Activity Excel Import" ' . $missingScopeActivityWriteDisablesButton . ' 
  title="generate/export activity Excel report
(requires caching first)"/>
	<input type="hidden" name="session" value="' . $s{ 'session' } . '"/>
	</form>
';
  say '
	...
	<input type="submit" name="submitFromNav" class="navButton" id="btnNavActList" value="Activity List " ' . $missingActivityCacheDisablesButton . ' 
...
(requires caching first)" />
	...
	<input type="submit" name="submitFromNav" class="navButton" id="btnNavActModify" value="Modify Activities" ' . $missingScopeActivityWriteDisablesButton . ' 
...
(requires write scope)"/>
	...
	<input type="submit" name="submitFromNav" class="navButton" id="btnNavFreqStartEnd" value="Frequent Locations" ' . $missingActivityCacheDisablesButton . '
...
sub geoBoxesFromDataFile {
  ...
  while ( my $line = <$fhIn> ) {
    ...
    my ( $continent, $country, $subdivision, $city, $latitude, $longitude ) = split ',', $line;
    $latitude  += 0;
    $longitude += 0;
...
sub geoBoxesFetchClosestEntry {
  ...
  my $minDist     = 999;
  my $minDistName = 'none';
  return ( $minDistName, $minDist ) if ( not defined $refBoxes->{ int $lat }{ int $lon } );
...
sub geoDistance {
  ...
  my $dist = sin( deg2rad( $lat1 ) ) * sin( deg2rad( $lat2 ) ) + cos( deg2rad( $lat1 ) ) * cos( deg2rad( $lat2 ) ) * cos( deg2rad( $lon1 - $lon2 ) );
