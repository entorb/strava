...
say "<form action=\"activityTop10.pl\" method=\"post\">
  ...
  <td>
  <select name=\"type\">
  <option value=\"Run\"" .  ( $type eq 'Run'  ? 'selected' : '' ) . ">Run</option>
  <option value=\"Ride\"" . ( $type eq 'Ride' ? 'selected' : '' ) . ">Ride</option>
  </select>
  </td><td>&nbsp;</td>
  </tr>
  <tr><td>Year</td>
  ...
  <select name=\"year\">";
foreach my $y (@years) {
	say "<option value=\"$y\"" . ( $year eq "$y" ? 'selected' : '' ) . ">$y</option>";
}
say "</select>
  </td>
  <td>
...
foreach my $column (@Reihenfolge) {
	...
	say "<th>Pace (min/km)" if ( $column eq 'km/h' );    # pace only for km/h ranking
	...
	foreach my $ref (@sorted) {
		...
		say "<tr>";
		say "<td>$count</td>";
		say "<td>" . TMsStrava::formatDate( $h{'start_date_local'}, 'date' ) . "</td>";
		say "<td>" . TMsStrava::activityUrl( $h{"id"}, $h{"name"} ) . "</td>";
		say "<td>$h{ $column }</td>" if ( $column ne 'x_min' and $column ne 'x_km' );
		say "<td>$h{ 'x_min/km' }</td>" if ( $column eq 'km/h' );    # pace only for km/h ranking
